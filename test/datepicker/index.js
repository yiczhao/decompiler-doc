...
;(function(global){
    ...
    function KS (options) {
        ...
        loader.config
            .then(function(configuration){
                ...
                // 已定义组件
                var moduleDefined = config(configuration,{
                                        loads:options.loads,
    ...
    var loader = {
        config:(function(){
            return new Promise(function(resolve,reject){
                 require(['./util/config','./util/require-css'],function(config) {
                    resolve(config)
                 })
            })
        })(),
    ...
    function configLoadDefined(args){
        var paths = {}
        var shim = {}
     ...
     * @return {[Array]}         [已注册的组件]
     */
    function config(args,loadAndKspath){
        
        args = args(loadAndKspath.kspath)
        console.log(args,loadAndKspath)
        Object.keys(loadAndKspath.loads)
            .forEach(function(key){
                args[key] = loadAndKspath.loads[key]
            })

        var pathsAndShim = configLoadDefined(args)
        ...
        var options = {
            ...
            map: {
              '*': {
                'css': './util/require-css' // or whatever the path to require-css is
              }
            },
